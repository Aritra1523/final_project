...
- **Redux Toolkit** + **react-redux** — global state for cart & products
- **Axios** — all network calls, isolated in `/api`
- **Tailwind CSS v4** — styling, with a small custom design-token palette
- **lucide-react** — icons
...
- Prices are shown in USD (`$`) to match the values returned by the
  dummyjson API.
- The "Checkout" button is a UI-only demo — there's no backend/payment
  integration.
