...
import { COUPONS, formatCurrency } from "@/lib/pricing";
import {
  applyCoupon,
  applyCouponCode,
  clearCoupon,
    ...
    <div className="coupon-box-wrapper">
      {appliedCoupon ? (
        ...
        <>
          ...
          {/* Clickable coupon chips - tap one to apply it instantly */}
          <div className="coupon-chips mt-3 flex flex-wrap gap-2">
              ...
              return (
                <button
                  ...
                  title={
                    eligible
                      ? c.label
                      : `Spend at least ${formatCurrency(c.minSpend)} to unlock`
                  }
                  className="coupon-chip flex flex-col items-start rounded-2xl border-2 border-indigo-200/60 bg-white/80 px-3.5 py-2 text-left shadow-sm shadow-indigo-100/30 transition-all duration-200 hover:-translate-y-0.5 hover:border-indigo-400 hover:shadow-md hover:shadow-indigo-200/40 active:scale-95 disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:translate-y-0 disabled:hover:shadow-sm"
                >
