...
// const initialState = {
//   items: [], // { id, title, price, category, thumbnail, quantity }
//   history: [], // [{ label, items }] snapshots for undo
//   couponInput: "",
//   appliedCoupon: null, // resolved coupon object from lib/pricing COUPONS
//   couponMessage: null, // { type: "success" | "error", text }
// };
...
//       state.couponInput = "";
//     },
//     // Called once, client-side only, after mount - restores whatever was
//     // saved in localStorage without ever touching the very first render.
...
// Shared by applyCoupon (typed input) and applyCouponCode (clicked chip) so
// both paths validate and set state the same way.
function applyCouponCodeLogic(state, code) {
  const coupon = COUPONS[code];
  const subtotal = state.items.reduce((sum, i) => sum + i.price * i.quantity, 0);
...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    applyCoupon: (state) => {
      ...
      applyCouponCodeLogic(state, code);
    },
